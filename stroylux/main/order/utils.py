...
from ..product.management import deallocate_stock, increase_stock
...
def update_order_prices(order):
    ...
    for line in order:  # type: OrderLine
        if line.variant:
            unit_price = line.variant.base_price
    ...
    if order.shipping_method:
        order.shipping_price = order.shipping_method.price
        order.save(
            update_fields=[
    ...
    for line in order:
        if line.variant:
            ...
            if line.quantity_fulfilled > 0:
                increase_stock(line, line.quantity_fulfilled)
