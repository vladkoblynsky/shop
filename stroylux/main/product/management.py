...
@transaction.atomic
def allocate_stock(
    order_line: "OrderLine", quantity: int,
):
    ...
    """
    stocks = (
        Stock.objects.select_for_update(of=("self",)).filter(product_variant=order_line.variant)
        .order_by("pk")
    )
...
def increase_stock(
    ...
    stock = (
        ...
        .filter(product_variant=order_line.variant)
        .first()
    )
    if stock:
        stock.increase_stock(quantity, commit=True)
    else:
        stock = Stock.objects.create( product_variant=order_line.variant, quantity=quantity)
    if allocate:
