...
def resolve_blog(info, id=None, slug=None):
    if id:
        return graphene.Node.get_node_from_global_id(info, id, BlogType)
    if slug:
        return graphene_django_optimizer.query(Blog.objects.filter(slug=slug), info).first()
    return None


def resolve_blog_list(info, ids, **_kwargs):
    user = info.context.user
    qs = Blog.objects.visible_to_user(user)

    qs = qs.distinct()

    if ids:
        db_ids = [get_database_id(info, node_id, BlogType) for node_id in ids]
