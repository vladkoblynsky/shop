...
from ..utils import get_nodes, registry
from .types import Error, Upload
from .types.common import AccountError
from .utils import snake_to_camel_case, from_global_id_strict_type
    ...
    return {
        error_type_field: graphene.Field(
            graphene.List(
                ...
                description="List of errorsAddressCreate that occurred executing the mutation.",
            ),
            default_value=[],
            required=True,
        )
...
def validation_error_to_error_type(validation_error: ValidationError) -> list:
    ...
    if hasattr(validation_error, "error_dict"):
        # convert field errorsAddressCreate
        ...
        # convert non-field errorsAddressCreate
...
class CreateToken(ObtainJSONWebToken):
    ...
    errors = graphene.List(
        ...
        required=True,
        deprecation_reason=(
            "Use typed errorsAddressCreate with error codes. This field will be removed after "
            "2020-07-31."
        ),
    )
    ...
    def mutate(cls, root, info, **kwargs):
        try:
            ...
            account_errors = [
                AccountError(
                    field="email",
                    message="Вы ввели не верный email и/или пароль",
...
class BaseMutation(graphene.Mutation):
    # errorsAddressCreate = graphene.List(
    #     graphene.NonNull(Error),
    #     description="List of errorsAddressCreate that occurred executing the mutation.",
    #     deprecation_reason=(
    #         "Use typed errorsAddressCreate with error codes. This field will be removed after "
    ...
    def clean_instance(cls, info, instance):
        ...
        try:
            ...
            if hasattr(cls._meta, "exclude"):
                # Ignore validation errorsAddressCreate for fields that are specified as
                # excluded.
                new_error_dict = {}
    ...
    def mutate(cls, root, info, **data):
        ...
        try:
            ...
            # if response.errorsAddressCreate is None:
    ...
    def handle_typed_errors(cls, errors: list, **extra):
        """Return class instance with errorsAddressCreate."""
        if (
...
class BaseBulkMutation(BaseMutation):
    ...
    def perform_mutation(cls, _root, info, ids, **data):
        ...
        for instance, node_id in zip(instances, ids):
            ...
            # catch individual validation errorsAddressCreate to raise them later as
            # a single error
            try:
                cls.clean_instance(info, instance)
            except ValidationError as e:
