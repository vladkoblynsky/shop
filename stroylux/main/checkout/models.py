...
from ..core.taxes import zero_money, zero_taxed_money, quantize_price
...
class Checkout(models.Model):
    ...
    def __str__(self):
        return f"{self.created} - {self.email}"
    ...
    def get_total(self):
        total = self.get_subtotal() + self.get_shipping_price() - self.discount
        return max(total, zero_taxed_money(self.currency))

    def get_subtotal(self):
        return sum([line.get_total() for line in self.lines.all()], zero_taxed_money(self.currency))

    def get_shipping_price(self):
        if not self.shipping_method or not self.is_shipping_required():
            return zero_taxed_money(self.currency)
...
class CheckoutLine(models.Model):
    ...
    def get_total(self):
        amount = self.quantity * self.variant.base_price
