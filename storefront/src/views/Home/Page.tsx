...
const Page: React.FC<{
	...
	const news: IArticleCard[] =
		articlesEdges?.map((edge) => ({
			id: edge.node.id,
		...
		<div className='home-page'>
			...
			<Container maxWidth='lg'>
				<Card className='my-20'>
					<CardContent>
						<Typography variant='h3' className='pt-20 pb-10 text-center'>
							Новинки
						</Typography>
						...
						{!newProducts?.edges.length && loading && <CarouselSkeleton />}
					</CardContent>
				</Card>
				<Card className='my-20'>
					<CardContent>
						...
						{!popularProducts?.edges.length && loading && <CarouselSkeleton />}
					</CardContent>
				</Card>
				{(articlesLoading || !!news.length) && (
					<Card className='my-20'>
						<CardContent>
							<Typography variant='h3' className='pt-20 pb-5 text-center'>
								Новости
							</Typography>
							<div className='separator max-w-300' />
							{!!news.length && <NewsCarousel news={news} />}
							{!news.length && loading && <CarouselSkeleton />}
						</CardContent>
					</Card>
				)}
			</Container>
