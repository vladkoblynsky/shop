...
import {
	...
	getOrderStatus
} from '@temp/views/UserProfile/pages/Orders/utils'
					...
					{!!order && (
						<>
							<Typography variant='h4' gutterBottom>
								<strong>Ваш номер заказа: {orderPk}</strong>
							</Typography>
										...
										{order.lines.map((line) => {
											return (
												<TableRow key={line.id}>
													...
													<TableCell>
														{line.variant?.attributes.map((attr, i) => (
															<span key={i}>
																{`${attr.attribute.name}: `}
																{attr.values.map((val) => (
													...
													<TableCell align='right'>
														{priceToString({
															amount:
																line.unitPrice.gross.amount * line.quantity,
															currency: line.unitPrice.currency
														})}
													</TableCell>
												</TableRow>
								...
								<div>
									<CardContent>
										...
										<Typography variant='body1'>
											<strong>Имя Фамилия:</strong> {address.firstName}{' '}
											{address.lastName}
										</Typography>
										<Typography variant='body1'>
