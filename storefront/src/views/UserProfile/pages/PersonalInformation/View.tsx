...
} from '@sdk/mutations/types/AccountUpdate'
import {
	accountEmailChangeMutation,
	accountPasswordChangeMutation,
	accountUpdateMutation
} from '@sdk/mutations/user'
...
import {
	...
	AccountPasswordChangeVariables
} from '@sdk/mutations/types/AccountPasswordChange'
import { AccountPasswordFormData } from '@temp/components/Forms/AccountPasswordForm/AccountPasswordForm'
import { useSnackbar } from 'notistack'
import {
	AccountEmailChange,
	AccountEmailChangeVariables
...
const View: React.FC = () => {
	...
	const [accountUpdate, responseAccountUpdate] = useMutation<
		AccountUpdate,
		AccountUpdateVariables
	>(accountUpdateMutation)
	const [passwordChange, responsePasswordChange] = useMutation<
		AccountPasswordChange,
		AccountPasswordChangeVariables
	>(accountPasswordChangeMutation)
	const [emailChange, responseEmailChange] = useMutation<
		AccountEmailChange,
		AccountEmailChangeVariables
	>(accountEmailChangeMutation)
	...
	const onSubmitPasswordChange = async (
		values: AccountPasswordFormData,
		resetForm
	) => {
	...
	const onSubmitEmailChange = async (
		...
		if (!errors.length) {
			resetForm()
			enqueueSnackbar(
				'Вам на текущий email отправлена инструкция для подтверждения',
				{ variant: 'success' }
			)
		}
	}
