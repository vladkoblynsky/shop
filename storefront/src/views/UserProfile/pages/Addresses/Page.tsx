...
interface PageProps {
    user: User | null,
    onSubmitAddressCreate: (values: AccountAddressFormData, cb: () => void) => void,
    errorsAddressCreate: AccountAddressCreate_accountAddressCreate_accountErrors[] | null,
    loadingAddressCreate: boolean,

    onSubmitAddressUpdate: (id: string, values: AccountAddressFormData, cb: () => void) => void,
    errorsAddressUpdate: AccountAddressUpdate_accountAddressUpdate_accountErrors[] | null,
...
const Page:React.FC<PageProps> = ({
                                      user, onSubmitAddressCreate,
                                      errorsAddressCreate,
                                      loadingAddressCreate, onSubmitAddressDelete, loadingAddressDelete,
    ...
    const [addressInitialData, setAddressInitialData] = useState<AccountAddressFormData | null>(null);
        ...
        <div>
            ...
            <div className={classes.container}>
                ...
                <div className={classes.addresses}>
                            ...
                            <div key={address.id} className={classes.address}>
                                <Card className="h-full flex justify-between flex-col">
                                    <CardContent>
                                        ...
                                        <p><span className="font-medium">Город: </span>{address.city}</p>
                                        <p><span className="font-medium">Адрес: </span>{address.streetAddress1}</p>
                                        {address.postalCode &&
                                        <p><span className="font-medium">Поштовый индекс: </span>{address.postalCode}</p>
                                        }
                                        {address.companyName &&
                                        <p><span className="font-medium">Компания: </span>{address.companyName}</p>
                                        }
                                        <p><span className="font-medium">Номер телефона: </span>{address.phone}</p>
                                    ...
                                    <CardActions>
                                        <ButtonGroup size="small" aria-label="address actions">
                                            <Button variant="outlined"
                                                    type="button"
                                                    size="small"
                                                    onClick={handleChangeAddress(address)}>Изменить</Button>
                                            <Button variant="outlined"
                                                    type="button"
                                                    size="small"
                    ...
                    {user.addresses.length === 0 && <Typography variant="h6">Список пуст</Typography>}
                </div>
                }
                    ...
                    <div className="flex my-10">
                        <ButtonGroup size="small" aria-label="address actions">
                            <Button variant="contained"
                                    ...
                                    type="button"
                                    size="large"
                                    onClick={handleAddAddress}>Добавить адрес</Button>
                        </ButtonGroup>
                    </div>
                </div>
                }
                <div>
                    ...
                    >
                        <DialogTitle id="address-form">{addressInitialData ? "Изменить адрес" : "Добавить адрес"}</DialogTitle>
                        <DialogContent dividers>
                            <AccountAddressForm onSubmit={addressInitialData ? handleSubmitAddressUpdate : handleSubmitAddressCreate}
                                                errors={addressInitialData ? errorsAddressUpdate : errorsAddressCreate}
                                                loading={addressInitialData ? loadingAddressUpdate :loadingAddressCreate}
