...
import {AccountAddressCreate, AccountAddressCreateVariables} from "@sdk/mutations/types/AccountAddressCreate";
...
import {AccountAddressDelete, AccountAddressDeleteVariables} from "@sdk/mutations/types/AccountAddressDelete";
import {useSnackbar} from "notistack";
import {AccountAddressUpdate, AccountAddressUpdateVariables} from "@sdk/mutations/types/AccountAddressUpdate";

const View:React.FC = () => {
    const user = useContext(UserContext);
    const {enqueueSnackbar} = useSnackbar();

    const [createAddress, responseCreateAddress] = useMutation<AccountAddressCreate, AccountAddressCreateVariables>(accountAddressCreateMutation);
    const [updateAddress, responseUpdateAddress] = useMutation<AccountAddressUpdate, AccountAddressUpdateVariables>(accountAddressUpdateMutation);
    const [deleteAddress, responseDeleteAddress] = useMutation<AccountAddressDelete, AccountAddressDeleteVariables>(accountAddressDeleteMutation);
    ...
    const onSubmitAddressUpdate = async (id: string, values: AccountAddressFormData, cb) => {
    ...
    return(
        <>
            ...
            <Page user={user.user}
                  ...
                  errorsAddressCreate={responseCreateAddress.data?.accountAddressCreate.accountErrors}
                  loadingAddressCreate={responseCreateAddress.loading}

                  onSubmitAddressUpdate={onSubmitAddressUpdate}
                  errorsAddressUpdate={responseUpdateAddress.data?.accountAddressUpdate.accountErrors}
