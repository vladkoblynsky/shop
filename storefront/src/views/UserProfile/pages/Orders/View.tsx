...
import {ProductReviewCreate, ProductReviewCreateVariables} from "@sdk/mutations/types/ProductReviewCreate";
...
const View:React.FC = () => {
    ...
    const { data, fetchMore, refetch } = useQuery<UserOrders, UserOrdersVariables>(userOrdersQuery, {variables: {
            first: PAGINATE_BY,
            filter:{
    ...
    const [createReview] = useMutation<ProductReviewCreate, ProductReviewCreateVariables>(productReviewCreate);
    ...
    const addReview = async (lineId:string, input: TProductReviewFormData, onSuccess) => {
        try {
