...
const CartItems:React.FC<CartItemsProps> = ({
    ...
    const {checkout, changeVariantQuantity, deleteCartLine, quantity} = useContext(CheckoutContext);
    ...
    const increaseVariantQuantity = async (variant: ProductVariant) => {
        ...
        await changeVariantQuantity(variant.id, validateAvailableVariantQuantity(line.quantity + 1, variant));
    };
    const decreaseVariantQuantity = async (variant: ProductVariant) => {
        const line = checkout.cart.find(el => el.variant === variant.id);
        await changeVariantQuantity(variant.id, validateAvailableVariantQuantity(line.quantity - 1, variant));
    };
    const handleChangeVariantQuantity = async (e:React.ChangeEvent<HTMLTextAreaElement | HTMLInputElement>, variant: ProductVariant) => {
        const val = e.target.value;
        try {
            if (val && parseInt(val) > 0 && parseInt(val) <= MAX_CHECKOUT_VARIANT_LINES){

                await changeVariantQuantity(variant.id, validateAvailableVariantQuantity(parseInt(val), variant));
            }
        }catch (e) {
    ...
    const validateAvailableVariantQuantity = (val: number, variant: ProductVariant): number =>{
        const variantStockQuantity = _.sumBy(variant.stocks,
            stock => stock.stockQuantity);
        const newQuantity = Math.min(val, variantStockQuantity, MAX_CHECKOUT_VARIANT_LINES);
        if (newQuantity < val){
    ...
    return(
        <div className="cart flex-1 py-10">
                    ...
                    return (
                        <div key={i} className="py-10 cart__item relative">
                            <div className="cart__item-delete-icon">
                                <IconButton onClick={e => deleteItem(node.id)}
                                            size="small"
                                            color="primary"
                                            title="Удалить">
                            ...
                            <div className="pb-10 flex flex-wrap">
                                <div className="mr-10">
                                    <img className="rounded-sm border border-solid border-gray-300" src={node.product.thumbnail.url}
                                         alt={node.name}/>
                                </div>
                                <div className="flex-1">
                                    ...
                                    <div
                                        className="cart__item-variant-name">
                                    ...
                                    <div className="cart__item-quantity">
                                        ...
                                        <span className="flex">
                                                                ...
                                                                <IconButton size="small"
                                                                            color="primary"
                                                                            disabled={localLine.quantity >= MAX_CHECKOUT_VARIANT_LINES}
            ...
            <div className="mt-20">
                ...
                <div className="cart__subtitle mt-2">Вы ничего не добавили в свою корзину. Мы уверены, что вы найдете что-то в нашем магазине</div>
            </div>
            }
        </div>
    );
