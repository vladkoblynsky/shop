...
}> = ({ attributes, filters, setFilters, initialCollapsed }) => {
	...
	const handleChangeAttributes = (attributeSlug, value) => (e) => {
		...
		if (filters.attributes) {
			...
			const index = _.findIndex(
				filters.attributes,
				(el) => el.slug === attributeSlug
			)
			if (index !== -1) {
				if (newAttributes[index].values.includes(value)) {
					newAttributes[index].values = _.filter(
				...
				<Collapse in={!collapse.includes('priceId')}>
					<Slider
						...
						onChangeCommitted={(e, newValue) => {
							setFilters({ priceRange: newValue as number[] })
						}}
			...
			{attributes && (
				<div>
					{attributes.edges.map((edge, idx) => {
						...
						const collapsed =
							initialCollapsed && idx < INITIAL_COLLAPSED_NUM
								? !collapse.includes(node.id)
								: collapse.includes(node.id)
						return (
							<div key={node.id}>
								<div
								...
								>
									<Typography variant='button'>
										<span className='normal-case'>{node.name}</span>
									</Typography>
									<div
										className='transition ease-in-out duration-300'
										style={{
								...
								<Collapse in={collapsed}>
									<Scrollbars autoHeight autoHeightMax={FILTER_HEIGHT}>
										{node.values.map((val) => {
											const selected =
												index !== -1
													? filters.attributes[index].values.includes(val.slug)
													: false
											return (
												<div key={val.id}>
													<FormControlLabel
														className='w-full'
														aria-label={val.name}
														onClick={handleChangeAttributes(
															node.slug,
															val.slug
														)}
														control={
