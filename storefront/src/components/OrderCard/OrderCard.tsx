				...
				<div className='flex xs:flex-col sm:flex-row flex-wrap'>
					<div className='flex-1'>
						<div className='flex flex-wrap xs:flex-col sm:flex-row py-5 px-10 sm:items-center'>
							...
							<div className='flex-1 text-center mr-5 text-gray-600'>
								{created}
							</div>
							...
							<Collapse in={isOpenOrder}>
								<div className='flex flex-col'>
											...
											<div className='py-10'>
												<div className='flex flex-wrap mb-5 px-10'>
													...
													<div className='flex-1 min-w-200'>
														{order.payments[0].paymentMethod?.name || 'Удален'}
													</div>
												</div>
												<div className='flex flex-wrap mb-5 px-10'>
													<div className='w-200 font-medium'>
									...
									<div className='py-5 px-10 flex flex-wrap justify-between items-center text-lg font-medium'>
										<div>
											Доставка (
											{order.shippingMethodName || order.shippingMethod?.name})
										</div>
										<div>
											{priceToString({
					...
					<div className='w-200 px-20'>
						<div className='font-medium py-10 '>
							{isOpenOrder ? 'Статус заказа' : DISPLAY_STATUS[order.status]}
						</div>
						<Collapse in={isOpenOrder}>
							<>
								<div>
									<div className={classes.status} data-status={order.status}>
										{DISPLAY_STATUS[order.status]}
									</div>
								</div>
								<div className='my-20 '>
									<NextLink href={STATIC_PAGES.returns.url} passHref>
										<Button
											component='a'
											variant='outlined'
											size='small'
