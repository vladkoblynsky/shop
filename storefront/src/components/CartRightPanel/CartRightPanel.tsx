...
const CartRightPanel:React.FC<{
    ...
    const {data:productVariantsData, fetchMore} = useQuery<ProductVariants, ProductVariantsVariables>(productVariantsQuery, {
        variables:{first: PAGINATE_BY, ids: checkout.cart.map(line => line.variant)},
            ...
            updateQuery: (previousResult, { fetchMoreResult = {} }) => {
                ...
                return {
                    productVariants: {
                        ...
                        edges: [...copy.productVariants.edges, ...fetchMoreResult.productVariants.edges],
                ...
                <div className="cart-panel__body">
                    <div className="cart-panel__header">
                        <div className="cart-panel__header-title">
                            Корзина <span>({quantity} товар{quantity > 1 && quantity < 5 ? 'а' : 'ов'})</span>
                        </div>
                    ...
                    <Scrollbars autoHide
                        ...
                        <InfiniteScroll
                            pageStart={0}
                            loadMore={loadFunc}
                            hasMore={productVariantsData?.productVariants.pageInfo.hasNextPage}
                            loader={<Loader key={0}/>}
                            useWindow={false}
                        >
                    ...
                    <div className="cart-panel__footer">
                        {quantity > 0 ?
                            <>
                                {sumPrice.amount > 0 &&
                                <Typography variant="h6" className="cart-panel__footer-total pb-10">
                                    <span>ИТОГО:</span>
                                ...
                                <Button type="button"
                                        ...
                                        color="primary"
                                        variant="contained"
                                        fullWidth>Оформить заказ</Button>
                            </>:
                            <Button type="button"
                                    component={Link}
                                    className="cart-panel__footer-btn mb-5"
                                    ...
                                    color="primary"
                                    variant="contained"
                                    fullWidth>Продолжить покупки</Button>
                        }
                    </div>
                </div>
            </SwipeableDrawer>
