...
} from '@sdk/mutations/types/CheckoutLinesUpdate'
import {
	CheckoutLinesAdd,
	CheckoutLinesAddVariables
} from '@sdk/mutations/types/CheckoutLinesAdd'
...
} from '@sdk/mutations/types/CheckoutLineDelete'
...
const CheckoutProvider: React.FC<{
	...
	const [quantity, setQuantity] = useState<number>(
		_.sumBy(checkout.cart, (line) => line.quantity)
	)
	...
	const authenticate = (token: string): void => {
		setLoading(true)
		client
			.query<Checkout, CheckoutVariables>({
				query: checkoutQuery,
	...
	const calculateCheckoutTotal = (
		items: ProductVariants_productVariants
	): number => {
		let amount = 0
	...
	const addVariant = async (variantData: TCartLine) => {
		const variant = checkout.cart.find(
			(el) => el.variant === variantData.variant
		)
		if (variant) {
			...
			if (checkout.id) {
				const lines = [
					{ quantity: variantData.quantity, variantId: variantData.variant }
				]
