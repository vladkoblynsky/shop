...
const ProductDetails: React.FC<ProductDetailsProps> = ({
	...
	const selectedVariantStockQuantity = selectedVariant
		? _.sumBy(selectedVariant.stocks, (stock) => stock.stockQuantity)
		: 0
	const stockStatus = selectedVariant
		? selectedVariantStockQuantity > 0
			? 'in'
			: 'out'
		: product.stockStatus
				...
				<div className='product-details__price font-medium'>
					{!selectedVariant && (
						<Typography variant='h3'>
							{showPriceRange(product.priceRange)}({product.unit})
						</Typography>
					)}
					{selectedVariant && (
						<Typography variant='h3'>
							{priceToString({
								...
								amount: Math.abs(
									selectedVariant.price.amount * selectedQuantity
								)
							})}{' '}
							({product.unit})
						</Typography>
