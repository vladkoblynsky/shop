...
} from '@sdk/mutations/types/ForgotPassword'
import { RESET_PASSWORD_REDIRECT_URL } from '@temp/constants'
...
const ForgotPasswordPage: React.FC<{
	...
	const [forgotPassword, forgotPasswordData] = useMutation<
		ForgotPassword,
		ForgotPasswordVariables
	...
	return (
		<div id='forgot-password-page'>
			<div className='my-10'>
				<IconButton size='small' onClick={back} title={'Назад'}>
					<AngelLeftIcon fillRule='evenodd' clipRule='evenodd' />
				</IconButton>
			</div>
			{!!successEmail && (
				<div>
					На ваш email <span className='font-medium'>{successEmail}</span>{' '}
					отправлена инструкция для сброса пароля.
				</div>
			)}
			{!successEmail && (
				<ForgotPasswordForm
