...
import {
	AccountRegister,
	AccountRegisterVariables
} from '@sdk/mutations/types/AccountRegister'
...
import { SIGNUP_REDIRECT_URL } from '@temp/constants'
...
const SignUpPage: React.FC = () => {
	...
	const [signUp, responseSignUp] = useMutation<
		AccountRegister,
		AccountRegisterVariables
	...
	const googleAuthKey = shop?.authorizationKeys.find(
		(key) => key.name === AuthorizationKeyType.GOOGLE_OAUTH2
	)
	return (
		<div id='signup-page'>
			{successEmail &&
				responseSignUp.data?.accountRegister.requiresConfirmation && (
					<div>
						<p>
							На вашу почту <span className='font-medium'>{successEmail}</span>{' '}
							отправлен email.
						</p>
						<p>
