				...
				<Sticky topOffset={xs ? 0 : 50}>
					{({ style, isSticky }) => (
						<Container
							maxWidth='lg'
							style={style}
							className={`bg-white ${isSticky ? 'shadow-lg' : ''}`}
						>
				...
				<div className='flex flex-1 flex-col'>
					<div>
						<div className='pl-24 py-5 flex items-center justify-between w-full'>
							{!authenticated && <Typography variant='h5'>Аккаунт</Typography>}
							{authenticated && (
								<Typography variant='h5'>
									...
									{!(user.user.firstName || user.user.lastName) && <>Аккаунт</>}
								</Typography>
							)}
					...
					{authenticated && (
						<div className='px-24 pb-15 mt-10 flex flex-1'>
							<AuthenticatedPage
								onClickLink={() => setAccountDrawerState(false)}
							/>
						</div>
					)}
			...
			>
				...
				<DialogContent dividers>
					<div className='mb-20'>
						...
						<a
							className='pl-5 hover:underline text-xl'
							href={`tel:${shop?.companyAddress.phone}`}
						>
							{shop?.companyAddress.phone}
						</a>
					</div>
					<div className='text-lg leading-10'>
						...
						<div className='pl-5 flex items-center justify-between'>
							<span>В будние:</span> <span>с 9:00 до 18:00</span>
						</div>
						<div className='pl-5 flex items-center justify-between'>
							<span>Суббота:</span> <span>с 9:00 до 15:00</span>
						</div>
						<div className='pl-5 flex items-center justify-between'>
							<span>Воскресенье:</span> <span>выходной</span>
						</div>
					</div>
				</DialogContent>
			</Dialog>
