...
import {CheckoutCreate, CheckoutCreateVariables} from "@sdk/mutations/types/CheckoutCreate";
import {CheckoutAddressUpdate, CheckoutAddressUpdateVariables} from "@sdk/mutations/types/CheckoutAddressUpdate";
import {CheckoutEmailUpdate, CheckoutEmailUpdateVariables} from "@sdk/mutations/types/CheckoutEmailUpdate";
...
const CheckoutAddress:React.FC<{
    ...
    const {checkout, resetCheckout, updateCheckout} = useContext(CheckoutContext);

    const [selectedAddress, setSelectedAddress] = useState<Address | null>(null);

    const [checkoutCreate] = useMutation<CheckoutCreate, CheckoutCreateVariables>(checkoutCreateMutation);
    const [checkoutAddressUpdate] = useMutation<CheckoutAddressUpdate, CheckoutAddressUpdateVariables>(checkoutAddressUpdateMutation);
    const [checkoutEmailUpdate] = useMutation<CheckoutEmailUpdate, CheckoutEmailUpdateVariables>(checkoutEmailUpdateMutation);
    ...
    const createCheckoutSubmit = async (values: TCheckoutShippingAddressFormValues) => {
        try{
            const response = await checkoutCreate({
                variables: {
                    input: {
                        ...
                        lines: checkout.cart.map(line => ({quantity: line.quantity, variantId: line.variant}))
                    }
                }
            });
    ...
    const updateAddressCheckoutSubmit = async (values: TCheckoutShippingAddressFormValues) => {
        try{
            ...
            const errors = [...response.data.checkoutShippingAddressUpdate.checkoutErrors, ...responseEmailUpdate.data.checkoutEmailUpdate.checkoutErrors];
            if (!errors?.length){
                updateCheckout(response.data.checkoutShippingAddressUpdate.checkout, responseEmailUpdate.data.checkoutEmailUpdate.checkout.email);
                history.push(activeStep.nextStepLink);
            }else{
                enqueueSnackbar(errors[0].message, {
                    variant: "error"
    ...
    return(
        <div>
            <div className="flex items-center justify-between mb-10">
                ...
                <Typography variant='h5' className="pr-10">Контактные данные</Typography>
                <Divider className="flex-1"/>
            </div>
                ...
                {user.user.addresses.map((address, i) => {
                    ...
                    return <div key={address.id}>
                        <FormControlLabel
                            ...
                            control={<Checkbox onChange={e => handleChangeAddress(address)}
            ...
            <ShippingAddressForm initialAddressData={checkout.address}
