...
import useWizard from "@temp/hooks/useWizard";
...
function canHitNext(
  ...
  switch (step) {
    ...
    case ProductVariantCreatorStep.prices:
      ...
      return !(data.stock.mode === "attribute" && data.stock.attribute === "");
...
function getTitle(step: ProductVariantCreatorStep, intl: IntlShape): string {
  switch (step) {
    case ProductVariantCreatorStep.values:
      return intl.formatMessage({ id: 'choose_values',
        defaultMessage: "Choose Values",
        description: "product attribute values, page title"
      });
    case ProductVariantCreatorStep.prices:
      return intl.formatMessage({ id: 'price_and_sku',
        defaultMessage: "Price and SKUs",
        description: "page title"
      });
    case ProductVariantCreatorStep.summary:
      return intl.formatMessage({ id: 'summary',
      ...
      >
        ...
        {step !== ProductVariantCreatorStep.summary ? (
          <Button
            ...
            onClick={nextStep}
          >
            <FormattedMessage id="next" defaultMessage="Next" description="button" />
          </Button>
        ) : (
          <Button
            className={classes.button}
