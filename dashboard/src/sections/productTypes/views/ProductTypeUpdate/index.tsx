...
import { attributeUrl } from "@temp/sections/attributes/urls";
...
import AssignAttributeDialog from "../../components/AssignAttributeDialog";
        ...
        <TypedProductTypeDetailsQuery displayLoader variables={{ id }}>
          {({ data, loading: dataLoading }) => {
            ...
            const handleAttributeAssignSuccess = (data: AssignAttribute) => {
              if (data.attributeAssign.productErrors.length === 0) {
                notify({
                  text: intl.formatMessage(commonMessages.savedChanges)
                });
                closeModal();
              } else if (
                data.attributeAssign.productErrors.length > 0
              ) {
                setErrors.addAttributeErrors(data.attributeAssign.productErrors);
              }
            };
            const handleAttributeUnassignSuccess = (
              data: UnassignAttribute
            ) => {
              if (data.attributeUnassign.productErrors.length === 0) {
                notify({
                  text: intl.formatMessage(commonMessages.savedChanges)
                });
                closeModal();
            ...
            const handleProductTypeDeleteSuccess = (
              ...
              if (deleteData.productTypeDelete.productErrors.length === 0) {
                notify({
                  text: intl.formatMessage({ id: 'product_type_deleted',
            ...
            const handleProductTypeUpdateSuccess = (
              ...
              if (
                !updateData.productTypeUpdate.productErrors ||
                updateData.productTypeUpdate.productErrors.length === 0
              ) {
                notify({
                  text: intl.formatMessage(commonMessages.savedChanges)
                });
                ...
                setErrors.formErrors(updateData.productTypeUpdate.productErrors);
              }
            };
                      ...
                      {!dataLoading &&
                        Object.keys(AttributeTypeEnum).map(key => (
                          <AssignAttributeDialog
                            ...
                            errors={maybe(
                              () =>
                                assignAttribute.opts.data.attributeAssign.productErrors.map(
                                  err => err.message
                                ),
                              []
                            )}
